# Project J -- Companion Assistant App Knowledge (v2, FULL)
...
items remain in v1.
2026-07-03: v2 FULL pass (Justin flagged the At-a-Glance hallucination). Full surface audit from
source. ADDED: every tab's header icons + FABs (incl. Otto's own sparkle FAB + name), At a Glance
+ Calendar Stats sections, the full 21-metric Trends list, Reports now names Day/Weekly/Monthly
Summaries, Log tab Intermittent Fasting + Edit Meals + nutrition gear, Gratitude Streak home card,
corrected card labels (Faith Today, Smart Tip), Faith Reading Plans + Devotionals sections, the
Workout/Food/Exercise Libraries, Head to Head, and a full ACHIEVEMENTS CATALOG (name + criteria per
family, with a hard "never state the user's live progress/count" guard + a summarize-and-route rule
for long lists). Paired with a strengthened no-guess rule in companionSystemPrompt.ts.
