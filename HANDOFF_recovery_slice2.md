...
1. **[BUG] Bedtime drill-down y-axis labels clipped on the LEFT** (device-confirmed, not fixed). Clock-time labels ("9:40 PM") overflow because MiniMetricChart PAD_L=32 is sized for short numeric ticks. Fix: widen PAD_L (~46) when chartValueFormat yields wide labels (bedtime only), or measure. components/MetricDrilldownModal.tsx MiniMetricChart. EASY first task.
