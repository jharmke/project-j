...
| E | Workout builder | 🟢 **FULLY DESIGNED 2026-08-13. `SPEC_workout_builder.md`. ALL 9 OPEN ITEMS ANSWERED. Nothing built.** ✅ **J's data is DONE, so E is no longer blocked on it.** ✅ **BOTH BLOCKING BUGS FIXED 2026-08-13**: the empty-library seed (spec 5.1) and the chat's auto-scroll (spec 2.1h, device-verified). 🔴 **BUILD ORDER CHANGED 2026-08-13: E now comes BEFORE item I, conditional on the preview showing muscles + instructions for a NEW exercise. See spec 8.1.** ➡️ **NOTHING IS BLOCKING THE BUILD NOW.** |
...
| J | Expand exercise library (79 -> 138) | 🔬 **THE 59 ENTRIES ARE WRITTEN 2026-08-13, NOT DEVICE-VERIFIED.** 4 of the 64 candidates cut by 4.18 item 12, **and a 5th cut on audit: `Farmer's Carry` was ALREADY in the library, which would have split PR history silently.** Mechanically checked: 138 unique ids, zero invalid muscle keys or tags, zero double dashes. **Still to do: device pass + equipment tags across all 138 (4.18 item 11).** Detail in `project_j_roadmap.md` item J. |
