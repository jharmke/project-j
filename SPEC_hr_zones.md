...
3. [NEXT] Settings (max HR override + model toggle). Justin confirmed keeping the manual max-HR override (standard in Garmin/Whoop/Apple/Polar). Engine already supports it (resolveMaxHR manualOverride, pj_settings.hrMaxOverride + hrZoneModel already read by the modal handler + dev dump). DISCLAIMER DECISION (pending build): a light inline hint under the field ("Most people can leave this on the estimate; only set your own if you know it from a test or race"), NOT a full first-use medical modal (the zone modal already carries the inline "Not medical advice" line). %max/Karvonen toggle: decide whether to even expose it (default Karvonen is the better choice; toggle is optional).
