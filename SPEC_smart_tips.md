...
- Standout picker: rank the day's Day Score sub-components (nutrition, activity, sleep, and their sub-parts) and surface the single most notable one, positive or corrective. "Most notable" is goal-weighted, not a raw deviation: the picker scans goal-importance tiers top down and applies absolute notable bands so it neither fakes drama on a flat day nor lets a low-importance metric headline. Full mechanic in TRIGGER_LIBRARY.md Section 12. Examples: "Sleep carried this day." "Strong day, protein was the one gap."
...
6. RESOLVED: the Day Takeaway engine rule set is written, TRIGGER_LIBRARY.md Section 12. Covers the goal-weighted standout picker (fill ratios, importance tiers per goal bucket, absolute notable bands, selection order, thin-data and balanced-day handling), the personal-history yardstick (streak / record / first-in-N with the today-is-the-noun guardrail), anti-repeat via the shared topic ledger, Mindful behavior with a positive-safe second pick to close the mode-switch hole (15.7 follow-on), secular faith (9.1), frozen deterministic storage, IDs, and full copy pools. Carries a HARD DEPENDENCY on the Day Score goal-snapshot + recompute-on-edit work (item 9 / roadmap): the frozen-snapshot determinism is not correct until that lands.
7. RESOLVED: Weekly and Monthly tip rules are written, TRIGGER_LIBRARY.md Section 13. Core principle: they REUSE the rolling domain rules (Sections 3 through 9) evaluated against a fixed completed period, not new thresholds. Weekly = one completed Sunday-Saturday calendar week (matches the existing Day Score archive grouping; rolling-7-day rejected because that is what EvR already is and a sliding window cannot be frozen), top 1-2 findings by priority, Urgent framing dropped for a past week. Monthly = one completed calendar month, weeks assigned by end-date, a trend fires when a metric was a weekly finding in 3+ of the month's weeks, framed as recurrence across weeks (altitude model 15.2). Covers diversity/topic-ownership via the shared ledger (15.3), pop-up firing (15.4), Mindful with positiveSafe pick (15.7), frozen deterministic storage with new weeklySnapshots/monthlySnapshots fields and the day-to-week-to-month recompute cascade (15.5), and altitude-specific copy templates for the headline metrics (remaining rules expand at build per the established copy convention). Carries the same HARD DEPENDENCY as item 6 on the Day Score goal-snapshot + recompute-on-edit work (item 9 / roadmap).
...
9. RESOLVED (verified in code): neither behavior exists today, confirming the 15.5 suspicion and Justin's forgotten-water example.
   - Goal snapshot: NOT stored. computeAndStoreDayScore (utils/dayScoreStore.ts) persists only the DayScore output object (composite, label, sub-scores, detail breakdowns, computedAt, version). The goal values (calTarget, paceTarget, proteinGoalG, waterGoal, activeCalGoal, stepGoal, weightGoal) live only on the transient input and are discarded after compute. Two nuances: the sub-scores are derived from those goals so the scoring math is effectively frozen, but the goal values are not recoverable from the record; and sleepGoal is the one value already snapshotted on the day record (day.sleepGoal), with calorie targets coming from the date-keyed loadCalorieTargets(dateKey) which may already read historical targets (worth a closer look at build, not snapshotted onto the score either way).
   - Recompute-on-edit: does NOT happen. ensureDayScore returns the stored score untouched when a current-version score exists, and the backfill scan skips any day that already has a current-version score. The only recompute triggers are a DAYSCORE_VERSION bump (global one-time rescan) or a day that has never been scored. There is no hook on food/water/workout/sleep edits to invalidate or recompute the affected day.
   - Spun off as its own Day Score build item (independent of Smart Tips, tracked in project_j_roadmap.md): (a) add a goal snapshot to the stored day record, freezing the goals in effect that day; (b) add a recompute-on-edit trigger so editing a past day's logged data recomputes that day, cascading to the week and month per 15.5.
