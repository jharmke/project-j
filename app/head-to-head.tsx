...
import { GOAL_DEFICITS } from '../utils/goalHit';
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (p.lifestyleActivity && p.trainingFrequency && p.weightGoal && p.birthday && p.heightFt && p.heightIn) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bgPrimary }}>
      ...
      <View style={{
        ...
        <TouchableOpacity onPress={() => { triggerHaptic(Haptics.ImpactFeedbackStyle.Light); router.back(); }} style={{ padding: 4 }}>
