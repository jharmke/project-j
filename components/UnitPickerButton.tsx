...
export default function UnitPickerButton({ value, options, onChange, minWidth = 48, embedded = false }: UnitPickerButtonProps) {
  ...
  const menuHeight = options.length * ROW_HEIGHT + MENU_VPAD;

  const runCascade = (up: boolean) => {
    panelOpacity.setValue(0);
    itemAnims.forEach(a => a.setValue(0));
    // Panel fades in quickly underneath the cascade so there's no hard pop.
    Animated.timing(panelOpacity, {
      toValue: 1, duration: 120, easing: Easing.out(Easing.cubic), useNativeDriver: true,
    }).start();
    // Cascade: opening down -> top-to-bottom; opening up -> bottom-to-top (closest to button first).
