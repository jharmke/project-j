...
NEVER use the project file reader after changes have been made to a file in the same thread. The project files are stale the moment a change is applied. Read from what Justin sent, or from the last confirmed changed state. If uncertain about current state, say so explicitly and ask Justin to repaste -- do not silently read the project file and assume it is current.
...
Big calorie number stays color codedFood Database
...
Session number increments by 1 every thread, no exceptions. Last Updated line must always have correct date AND correct session number. Never copy previous session number forward. Current session is tracked by counting threads -- if unsure, ask Justin before writing the Last Updated line.
...
Always send git commands explicitly when telling Justin to commitWhat Justin Needs From Every New ThreadKnow the find/replace format from message one
