...
Session number increments by 1 every thread, no exceptions. Last Updated line must always have correct date AND correct session number. Never copy previous session number forward. CRITICAL -- Claude cannot count threads reliably across accounts and time. The FIRST thing Claude does in every new thread, before reading roadmap, before responding, before anything else -- ask Justin: "What session number is this?" One question, no exceptions. Do not assume, do not count, do not guess. Wait for Justin to confirm before proceeding.
