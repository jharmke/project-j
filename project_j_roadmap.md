...
[ ] Achievements audit -- IN PROGRESS. Status per category:
  ...
  Momentum: DONE (8 achievements, renamed from Streak). 3/7 Bronze, 14/30 Silver, 60/90 Gold, 180 Platinum, 365 Diamond. checkMomentumAchievements() standalone function in achievementData.ts -- fires from food-detail.tsx (primary), journal.tsx, index.tsx (fallback). Once-per-day gate only arms after today has pj_YYYY-MM-DD data (prevents journal-before-food swallowing the gate).
  General: 3 achievements (Day One, First Word, Consistent Voice). First Word + Consistent Voice moved from faith category -- now count personal/fitness/gratitude/workout journal entries only (verse/prayer/study excluded). Trigger wired in journal.tsx createEntry. NEEDS EXPANSION PASS next general session -- add journal milestones (25/50/100/365), Days in App milestones, any other general firsts.
  Faith: EMPTY. Journal achievements pulled out -- needs dedicated design session. Planned content: verse reflection milestones (1/10/25/50), Bible reading streak achievements, prayer log milestones, gratitude streak milestones. Do NOT build until names/descriptions are picked same process as Momentum session.
  Workout, Sleep, Nutrition: NOT STARTED. Each needs dedicated naming/description session before any code.
  Diamond tier: Falls through to Large celebration until dedicated animation built. Spec: screen dims, full-screen trophy pop, completely distinct experience. AchievementToast.tsx diamond TIER_CONFIG entry added (ice blue), isPlat check updated to include diamond -- crash bug fixed.
...
Tutorial / walkthrough system -- interactive overlay style (live UI underneath, highlighted circles, callout bubbles, tap-to-advance). Mode-aware: each mode gets its own tutorial voice and emphasis. Premium feel -- this should feel like a product moment, not a tooltip tour. In-depth but digestible: covers the why, not just the how. Opt-in from settings and accessible via toolkit icons on screens with a built tutorial.
  Meta-tutorial fires on first open for ALL coaching styles (not Mindful only) -- very brief, quickly skippable. Purpose: teaches WHERE to find tutorials (the icon/button on each card), not the tutorials themselves. Prevents day-one info overload. After meta-tutorial, user is released into the app -- individual card tutorials are user-initiated and discovered at their own pace.
  Individual tutorials are interactive (live UI underneath, highlighted circles, callout bubbles, tap-to-advance) -- not static info cards, those are the toolkit. Each card/feature gets its own tutorial accessible via icon or hotlink in the toolkit. First real card tutorial does not auto-play.
  Immediate skip/close always available. Goal: really polished, feels intentional -- a UX differentiator. Needs dedicated planning thread before building.
