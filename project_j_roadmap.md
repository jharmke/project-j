...
NOW -- active this session
  ...
  [ ] BUG: Sleep score inconsistency -- home card, day detail, YvY, and head-to-head all compute sleep score independently and produce different results for the same night. Observed 2026-05-29: home card showed 93 for previous night; day detail and YvY both showed 83 for same night; head-to-head showed 60. Three separate code paths, three diverging results.
    Root cause analysis: (1) Home card computes live from raw HealthKit stage data while the night is "today" -- full stage fields (deep %, REM %, duration) in memory, no dependency on stored values. (2) Day detail recomputes the next day from persisted pj_YYYY-MM-DD fields -- if any stage data did not persist correctly, or uses a slightly different field name or default value, score diverges. (3) Head-to-head diverges most severely (60 vs 83 vs 93) -- third independent computation where a stage field likely arrives as 0 or null, gutting the deep or REM component of the score entirely. (4) YvY matches day detail exactly (both 83), confirming they share the same stored-data computation path -- that path is internally consistent but still diverges from the live home computation.
    Correct fix: compute sleep score once (at the point HealthKit data is finalized for the night), store the result in pj_YYYY-MM-DD.sleepScore alongside the raw stage percentages, and have all downstream views (day detail, YvY, head-to-head, any future report) read the stored score rather than recomputing independently. If stored score is absent on historical data, fall back to on-demand computation from stored stage fields -- but all fallback paths must use the exact same formula with the exact same field names and defaults. Single source of truth, zero divergence.
    Files to audit: index.tsx (home live computation + YvY stored computation), day-detail.tsx (stored computation), head-to-head.tsx (stored computation). Confirm all three call sites use identical inputs from identical fields. The 23-point gap between home (93) and head-to-head (60) is the tell -- something is missing or zeroed in the head-to-head path. The 10-point gap between home and day detail/YvY suggests a field that exists in memory but does not persist correctly. (NOW)
  [ ] BUG: Graph data accuracy sweep -- net calories graph shows different values than day detail for the same dates. Observed example: graph showed 905, day detail showed 695 for the same day -- a 210 kcal divergence that is not rounding error. Suspected root cause: fetchTrendData in utils/statsData.ts uses a stale or zero BMR value at call time, while day detail computes BMR inline from actual profile data loaded fresh on mount. Cannot confirm until utils/statsData.ts is audited -- that file was not included in prior project uploads.
    When fixing net cal, do a full sweep of ALL graph data types computed in fetchTrendData: calories consumed, macros (protein/carbs/fat individually), steps, active cals, sleep hours, sleep score, weight, net cals, resting HR, workout frequency, effort score. For each data key: (a) confirm it reads from the correct field in pj_YYYY-MM-DD (exact field name match to what day detail reads), (b) confirm burn accuracy modifier is applied identically to how day detail and achievement logic apply it -- no raw HealthKit active calorie values in any computation, (c) confirm BMR is computed from fresh profile data loaded at call time, not from stale React state that may be zero on initial mount. Do not assume any graph type is correct until manually verified against day detail for the same date range. Stats graph values must agree with day detail for the same day. (NOW)
