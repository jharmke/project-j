...
[x] Our Mission screen -- SHIPPED. app/mission.tsx. 5 cards: The Foundation, You vs Yesterday, We Do the Math Right (burn accuracy + running BMR combined), The App Adapts to You (coaching modes), Faith is in the Foundation. Hero OUR MISSION title 48px Bebas accent in scroll content. Card headlines accent colored. Vertical free scroll. Entry points: Settings > Help > Tips & Guides row + all-set.tsx hotlink. All 5 cards visible for all Faith Journey tiers including NRN. Dedicated copy + visual pass still needed: voice rule = "we" for conviction only (faith card), second person or no subject for functional cards.
Diagnostic Report -- "Why am I not losing weight?" style report. Compares expected results vs actual results across key variables: logged calorie deficit vs actual weight change, Apple Health burn accuracy (flags if overestimate likely), logging consistency rate (days with <400 cal logged = suspect), macro distribution quality, sleep quality correlation. Framed gently -- not accusatory, not medical advice. Closes with 2-3 highest-leverage suggestions. Mindful version softens all language. This is a major differentiator -- no other consumer app connects the dots this honestly. Needs dedicated planning session for formula and UX before building. (SOON)
