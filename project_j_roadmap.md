...
NOW -- active this session
  ...
  [x] NetCalBarChart -- zero baseline SHIPPED 2026-05-29. New components in StatsGraphCard.tsx. The fix above exposed that net cal is correctly mostly negative (deficits), which the old GenericBarChart could not render (squashed deficits to slivers, one surplus day towering). Built per the locked spec:
    ...
    Remaining graph sweep (lower priority, separate pass): macros (P/C/F), steps, weight read correct fields directly -- spot-confirmed during audit. OPEN DECISION: Active Calories graph + At a Glance avg active cals still show RAW HealthKit numbers (no burn accuracy), while net cal and the home card "Active" sub-stat show adjusted. Decide whether active cals should apply accuracy (match home card) or stay a raw measurement. Part of burn accuracy audit (SOON item 8). (NOW -- net cal done, active cals decision open)
