...
SOON -- confirmed next few sessions
  ...
  [~] Calorie goal hit logic overhaul -- STATUS: IN PROGRESS (started 2026-05-29). Full build doc: SPEC_calorie_goal_hit.md (decisions locked, do not re-litigate). AUDIT FINDING 2026-05-29: goal-hit is currently computed 6 divergent ways, not 3. Correction to the prior note below: the achievement trigger does NOT do it correctly. It uses RAW active calories (calTarget + data.activeCalories, no burn-accuracy modifier), which is itself a burn-accuracy bug. The 6 sites: (1) achievementData.ts checkNutritionAchievements, (2) home calorie card index.tsx, (3) stats.tsx calendar dots, (4) stats.tsx At a Glance CAL GOAL/DAY count, (5) stats.tsx calorie streak, (6) log.tsx calorie color. This is a rewrite to one shared util (utils/goalHit.ts), not a reconcile.
    ...
    BUILD PROGRESS (2026-05-29): utils/goalHit.ts shared util created. Wired: site #1 achievementData.ts (raw-active bug killed), site #2 home calorie card color (index.tsx, green now means a real Way 1/Way 2 hit via calorieColorTier), site #4 stats At-a-Glance CAL GOAL/DAY count, site #5 stats calorie streak. Open verification #2 RESOLVED: home card hkCalories already applies burn accuracy (rawHkCalories * burnAccuracyPct / 100), no bug. calorieColorTier made case-insensitive (index uses lowercase styleMode, stats capitalized). DEFERRED: site #3 stats calendar day-cell coloring (Justin on the fence, not worth the time/usage right now). The On Target/Close/Off legend pills are HIDDEN as of 2026-05-29 (cell-tint code left in place, restore legend via git if revisited). Side note found while there: the calendar day-cell tint appears invisible in-app even with logged data (suspect calTarget unread at render or sub-0.25-alpha tint washing out on Light theme). Not chased, parked with site #3. Remaining: site #6 log.tsx color (Step 5), net consolidation in head-to-head (Step 6), diagnosticReport burn-accuracy fixes (Step 7).
