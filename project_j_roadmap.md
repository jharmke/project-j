...
Tutorial System -- full spec in tutorial_system_spec.md. Architecture + design session COMPLETE 2026-05-23. All visual and functional decisions locked in spec. Two distinct things: Toolkit (static help panel, definitions) and Tutorial (interactive overlay, live UI, callout bubbles, NEXT button to advance). Tab-level toolkit icon far left of each tab header. Card-level Take a Tour button near top of (i) modals. Settings > Help > Tutorials section lists all. Meta-tutorial fires once for all users, teaches WHERE to find tutorials not the tutorials themselves. All tutorials skippable. Mode-aware voice (Discipline/Balanced/Mindful variants per step). Build order: engine (TutorialOverlay + TutorialContext) → tab icons → Take a Tour in (i) modals → Settings > Help entry → meta-tutorial → individual tutorials in priority order. All tab audits complete -- see spec doc.
