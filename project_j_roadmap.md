...
NOW -- active this session
  ...
  [ ] Interactive Tutorial Engine + Log Tab -- IN PROGRESS. Full plan confirmed. 8 files. No home tab or existing tutorial changes. See tutorial_system_spec.md for full spec. Engine upgrades: TutorialStep gets navigateTo + tutorialAction fields, TutorialContext gets registerTutorialAction/unregisterTutorialAction callback system, TutorialOverlay gets navigation-aware measureTarget with retry loop (5x, 150ms apart, ~750ms window). Tutorial data isolation: tutorialMode:true bypasses all achievements/Firestore/streaks/celebrations on save; tutorialEntry:true marker on entry for crash recovery; launch-time scan in _layout.tsx nukes orphaned entries silently. Tutorial food: data/tutorialFood.ts hardcoded Grilled Chicken Breast (165 kcal, 31g protein, 0g carbs, 3.6g fat, 4 serving sizes, full extended nutrition -- USDA values). log_food tutorial: 11 steps, fully interactive, navigates log -> add-food -> food-detail -> back to log. Food actually saved and actually deleted, zero data footprint. manage_log: auto-logs tutorial food if log empty, walks through edit/delete, auto-deletes. 13 refs wired across log.tsx, add-food.tsx, food-detail.tsx.
...
SOON -- confirmed next few sessions
  ...
  Tutorial spotlight target wiring (workout, stats, profile) -- Log tab fully wired as part of interactive tutorial engine work. Workout/stats/profile are single-screen tabs, no interactive engine needed -- just useTutorialTarget('key') + ref={ref} + collapsable={false} on target Views. Keys: workout_day_scroller, workout_exercise_row, workout_sets_reps, workout_cardio_fields, workout_progress_count, workout_effort, stats_fab, stats_streaks_section. Profile tutorials are all targetKey:'none' by design.
