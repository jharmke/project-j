...
- [NOW, surfaced 2026-07-22] **Serving-unit redesign sweep -- finish it across all three food surfaces.**
  ...
  (4b) OPEN, NEXT -- **edit an ingredient's amount in the recipe builder.** Today an ingredient is
      delete-and-re-add only. Plan (aligned with Justin 2026-07-22): tap the ingredient ROW (not a small
      pencil; the X stays separate) -> centered modal with the amount + the same unit picker, SAME-FAMILY
      units only (a grams ingredient can't jump to cups without density). Every nutrient on an ingredient
      is a straight linear multiple of its amount, so rescaling is exact, not an estimate.
