...
[ ] Food log past date nav bug -- after adding food on a past date, back navigation from add-food bounces app to today. Should stay on viewed date. Only revert to today on tab switch or full app backgrounding, not on back navigation. app/(tabs)/log.tsx.
[ ] FatSecret search path missing food.get -- normalizeFsSearchResult parses 4 fields from description string only, never calls food.get. Extended nutrition (fiber, potassium, poly/mono fat, vitamins, minerals) only populates via barcode path. Root cause: fiber always 0 on text-searched foods. Fix: call food.get after text search selection, same as barcode path. add-food.tsx.
[ ] Cardio decimal input limit -- edit cardio workout fields only allow 1 decimal place (3.1 max). Should allow 2 (3.11, 5.22). filterDecimal needs updating for distance, speed, and related cardio numeric fields. workout.tsx.
