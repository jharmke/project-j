...
- [ ] Expand Otto's on-demand data access (ONE DATASET AT A TIME, same conditional-injection pattern as PRs / utils/companionPRs.ts): give Otto more of the user's real data, attached only when the message is about it. DONE: PR values + real-exercise recognition + per-lift TREND (A) + recent-workouts B (30-day session summaries) + food log history (30-day per-day totals + itemized named-day foods) + sleep + recovery history (30-day per-night + named-night stage/signal detail) + body measurements (full history: current per-field value/age/delta + Navy BF% + session history) + achievements (earned set by category + live progress toward every badge family, via shared achievementProgress scan; streaks intentionally deferred). REMAINING: journal/prayer entries (PRIVACY-sensitive, handle carefully; the LAST one in the thread) · later: full per-metric streak tiles (needs the Stats streak engine extracted first). Do NOT build true LLM tool-use yet (Haiku too flaky + extra round-trips); revisit only if Otto grows into a full data assistant on a stronger model. Build/verify each one at a time.
