...
Profile screen save bar overlap -- when keyboard is up and a change triggers the floating save bar, the save bar overlays the focused TextInput and the scroll doesn't adjust. Needs: track current scroll offset via onScroll, detect hasChanges becoming true while keyboard is active, bump scroll by SAVE_BAR_HEIGHT (~76px). scrollToEnd() approach was tried and reverted -- it auto-scrolled to bottom for ALL fields not just goal inputs. (NEXT SESSION)
Sleep Goal picker bug -- opening the Sleep Goal CollapsibleCard immediately triggers hasChanges without any user interaction. Root cause: the SleepGoalPicker's programmatic scrollTo fires during the CollapsibleCard expand animation, which triggers onScrollEndDrag at position 0 (5h) before the scroll reaches the correct position, calling onChange with wrong value. Fix: add isInitializing ref in SleepGoalPicker to block onChange during the initial scrollTo. (NEXT SESSION)
