...
[ ] Celebration particles not showing -- root cause: particles were created in useEffect (after render) but stored in a ref (no re-render triggered), so render always saw empty array. Fixed in code: moved particle creation into useMemo so they exist synchronously before first render. useEffect now just resets values and starts animations. CelebrationOverlay.tsx. NEEDS BUILD VERIFICATION.
[ ] Step goal trigger race condition -- prevStepsRef.current was updated even when loaded=false (same bug previously fixed for activeCals and exerciseMins but missed for steps). HealthKit delivered steps before AsyncStorage loaded, ref got set to real value, crossing check failed when loaded became true. Fixed in code: steps trigger and prevStepsRef update now both gated inside if (loaded) block. index.tsx. NEEDS BUILD VERIFICATION.
[ ] Height field validation -- feet clamped 1-8, inches clamped 0-11, non-numeric input stripped, maxLength enforced. Was possible to enter 522ft 29in. Fixed in code: profile.tsx. NEEDS BUILD VERIFICATION.
...
[ ] Settings section label color -- all CollapsibleSection headers (Goals, Faith & Style, Health, etc.) use textMuted, looks weak compared to sub-headers inside. Change to accentBlue. settings.tsx.
[ ] Settings collapsible close animation choppy -- opacity-only fade then instant height snap is jarring on tall sections (Goals, Notifications). Implement proper onLayout-measured height animation (JS thread) + content opacity/translateY (native thread), same dual-animation standard. settings.tsx.
[ ] Settings Help definitions list -- 20+ tooltip entries dumped as flat scroll inside Help section. Replace with single tappable "View Definitions" row that routes to a dedicated definitions screen. Keeps Show Again functionality, cleans up Help. settings.tsx + new screen.
