...
SOON -- confirmed next few sessions
  ...
  Barcode tutorial -- NEXT. Full interactive redesign (not guided tour). 5-step flow: (1) navigate to add-food via navigateTo, spotlight barcode icon in header; (2) tutorialAction 'showTutorialScanResults' in add-food.tsx injects hardcoded fake scan results + sets fake lastScannedBarcode, spotlight top result with green checkmark -- "if this is the right food, tap it like any other"; (3) spotlight SET button on a result row -- "not right? search or browse, tap SET to link it permanently"; (4) tutorialAction 'switchToSetFoodsTab' switches to Set Foods tab, spotlight UNSET button; (5) tutorialAction 'showTutorialNoMatchState' clears results / keeps fake barcode active, spotlight "Create Food for this Barcode" footer button. Requires 3 new tutorial actions registered in add-food.tsx. Cleanup: tutorialAction 'clearTutorialScanState' on last step DONE + skipTutorial path.
  create_food: should open CustomFoodCreator, walk through fields, save, then delete on DONE.
  recipes: should open recipe builder, walk through, save, spotlight result, delete on DONE.
