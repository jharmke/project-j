...
[ ] TUTORIAL POLISH PASS -- dedicated session required. Confirmed good: spotlight animation quality, highlighting, interactive flows, app stability. Issues to fix as a batch:
  ...
  [ ] Auto-scroll to off-screen targets -- PARTIALLY FIXED 2026-05-26. doEntry non-navigation branch now uses measureTargetWithRetry (helps preAction ref timing). Still broken on manage_log last step (TODAY'S TOTAL): element scrolled above viewport, measureInWindow returns zeros on iOS for off-screen elements, isOffScreen(null)=false, scroll skipped. Need to handle null-but-ref-registered case in doStepTransition by attempting scrollToTarget before giving up. TutorialOverlay.tsx.
