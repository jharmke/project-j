...
[x] Empty day state on workout tab -- ACTIVE day with zero exercises shows card with Load Routine button and Add Exercise button (opens library in select mode). workout.tsx.
...
IF window closing notification -- local notification scheduled when IF timer starts: fires at 60 min remaining ("Last hour of your eating window. Anything else you need?") and at 30 min remaining ("30 minutes left to eat today."). Cancels if user taps LAST MEAL before threshold. Depends on push notification infrastructure.
