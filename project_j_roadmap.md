...
- Sleep Hub baseline spec -- NEEDS DEDICATED SESSION. Personal rolling baselines (30-day avg) for sleep metrics: avg bedtime, avg deep %, avg REM %. Show alongside healthy ranges in Sleep Metrics panel. Bedtime consistency specifically blocked on this -- +-14m meaningless without baseline context. Healthy range subs ("Healthy 13 to 23%") need redesign once baselines exist. Full dim text cleanup across Sleep Metrics rows also deferred here -- baseline context replaces the need for dim subs. app/sleep.tsx + useHealthKit.ts.
