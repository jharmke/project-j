...
NOW -- next session
Profile screen KAV -- goal TextInputs near bottom of screen get covered by keyboard. Use automaticallyAdjustKeyboardInsets={true} on the ScrollView (iOS native, no KAV wrapper needed). Evaluate first, do not blindly add KAV -- prior KAV attempts on complex scroll screens caused layout issues.
