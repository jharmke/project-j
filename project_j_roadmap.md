...
[ ] Water streak retroactive recalculation -- logging water on a past day that pushes it over goal does not update the streak. Streak engine only fires on live save, not on historical edits. Need to audit streak update logic app-wide: any time a pj_YYYY-MM-DD key is written for a past date, the relevant streaks (water, calories, steps, etc.) should re-scan from that date forward and correct the streak count. (SOON)
