...
[ ] Effort vs Results full flow redesign -- split screen into two distinct pages. Setup/List page: window picker (14/30/90) with date range shown, dimmed window button shows inline reason ("11/15 days logged"), Generate Analysis button lives here only, list of historical reports below with CURRENT badge on most recent + trash icon on each with delete warning toast, flat cap of 15 reports (oldest auto-deleted when hitting 16, no warning). Report View page: clear header showing window duration + date range, no setup/no window pickers/no generate button -- report content only, delete button that toasts "Report deleted" and kicks back to list automatically. Tap any report from list to load it. app/diagnostic-report.tsx. (SOON)
