...
Custom streaks system -- all three modes get full access to all streak options. User can create custom streaks with name and emoji, check in manually. Defaults differ per mode: Discipline/Balanced default to performance metrics (calories, steps, workout). Mindful defaults to gentle habit-forming ones (gratitude entry, hydration, morning intention). Architecture is the same across all modes. NRN users: Bible streak hidden, replaced with a custom slot the user can assign. All streak types must be configurable so NRN users can set meaningful goals without faith-based defaults.
