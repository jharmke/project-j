...
[x] Tutorial System -- ENGINE SHIPPED 2026-05-23, POLISH PASS 2026-05-23.
  ...
  Spotlight target wiring: Engine + hook built. useTutorialTarget NOT yet wired to actual card Views -- tutorials show overlay + bubble but no cutout highlight. Full-screen dim is graceful degradation. Wiring individual cards is next session work.
  AsyncStorage key: pj_tutorials.

SOON -- confirmed next few sessions
  Tutorial spotlight target wiring -- Wire useTutorialTarget('key') to actual card Views in all 5 tabs so tutorials show real cutout highlights. Home tab first: cal_card_main, macros_card_main, sleep_card_main, if_card_main, yvy_card_main. Then log, workout, stats, profile. Each card View gets ref={ref} collapsable={false}. Engine is complete -- this is pure wiring work per card.
