...
- [BUG] Sleep Stages Y-axis gridlines not equidistant -- 8h-9h band is visually compressed vs lower bands. Axis scaling calculation needs a fix so all gridline intervals are equal. app/sleep.tsx Hypnogram / stacked bars.
- Hypnogram connector lines -- link floating stage bars with thin vertical transition lines to create an unbroken sleep wave. Primary: LinearGradient step-down (departing color to arriving color, Apple Health style). Fallback: low-opacity gray if gradient proves messy. app/sleep.tsx Hypnogram component.
- Hypnogram Y-axis label color-coding -- text color of Awake/REM/Core/Deep labels should match each stage's block color exactly (Awake #cc3333 / REM #7e6bb3 / Core #3d6e9e / Deep #0d9268). app/sleep.tsx Hypnogram.
- Awake legend row in stage key -- add red color bubble + total awake duration to the bottom key row, matching the Core/Deep/REM format. Remove the "8m awake last night" subtitle under Wake events (redundant once Awake is in the key). app/sleep.tsx.
- Wake events row consolidation -- wake count (7) and total awake time are separated (sub under label on left, count on right). Both belong together on the right side. Show count + total awake time in the value area. Exact separator format TBD at implementation. app/sleep.tsx metrics row.
- Bedtime consistency human-readable reformat -- "+/-14m" alone is meaningless. Reformat to something like "Bedtime varies +/-14m" or "Bedtime window: +/-14m from 9:41 PM" to anchor the number to something recognizable. Exact copy TBD at implementation. app/sleep.tsx metrics row.
- Sleep Metrics healthy range / baseline spec -- "Healthy 13 to 23%" sub placement is ambiguous: user can't tell if it's a healthy target or their personal baseline. Direction: value large on right, healthy range in small muted text directly beneath it. OPEN QUESTION: should personal baselines also surface here? Healthy range vs personal baseline are different things. Needs dedicated spec discussion before any build. Do not touch until resolved.
- Exclude card polish -- the exclude-last-night toggle reads like a lost footnote (no title, no context). Options: give it a proper card label (e.g., "OPTIONS" or "DATA"), or fold the toggle into the Sleep Metrics card as a bottom row. TBD at implementation. app/sleep.tsx.
