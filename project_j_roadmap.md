...
SOON -- confirmed next few sessions
  ...
  Log tab interactive demos -- manage_log / create_food / recipes are currently powerpoint (spotlight refs wired but no demo state injected). All three need the same preAction/tutorialAction treatment as log_food and workout_basics: inject demo data before step 0, clean up on DONE/SKIP. manage_log: needs demo food entries already logged so entry rows exist to spotlight. create_food: should open CustomFoodCreator, walk through fields, save, then delete on DONE. recipes: should open recipe builder, walk through, save, spotlight result, delete on DONE. barcode: camera-hardware-dependent, acceptable as guided tour -- no interactive demo. Next session priority.
