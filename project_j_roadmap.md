...
[ ] Achievements audit -- IN PROGRESS. Status per category:
  ...
  Nutrition: NAMING IN PROGRESS. Spec locked: 8 achievements (1/10/30/50/75/100/200/365 calorie goal hit days). Goal hit = consumed within -300/+150 of adjustedTarget (calTarget + hkCalories). Must have >= 1 food entry logged. Trigger uses === exact match on hitCount so each achievement fires on exactly one day. Color palette: teal (#0d9268 family). Draft names from session: In Range / Dialed / By the Numbers / The Formula / Locked In / Optimized / Machine / Fueled -- NOT finalized. Resume naming next thread, then build checkNutritionAchievements() and wire triggers.
