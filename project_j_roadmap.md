...
Tutorial System -- full spec in tutorial_system_spec.md. Two distinct things: Toolkit (static help panel, definitions) and Tutorial (interactive overlay, live UI, callout bubbles, tap-to-advance). Tab-level toolkit icon far left of each tab header. Card-level Take a Tour button near top of (i) modals. Settings > Help > Tutorials section lists all. Meta-tutorial fires once for all users (trigger TBD), teaches WHERE to find tutorials not the tutorials themselves. All tutorials skippable. Mode-aware voice (Discipline/Balanced/Mindful variants per step). Build order: architecture → tab icons → Take a Tour in (i) modals → Settings > Help entry → meta-tutorial → individual tutorials in priority order. Home + Log tab audits complete -- see spec doc. Workout/Stats/Profile audits still needed.
