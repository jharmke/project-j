...
SOON -- confirmed next few sessions
  ...
  [ ] BUG: Food detail time-logged scroller not centered -- sits left-aligned (like the old settings scrollers before they were fixed). Center it. food-detail.tsx. (SOON)
  [ ] BUG: Food detail AVG SERVING shows "g" regardless of the unit actually used -- e.g. Muscle Milk default is 330ml but the AVG SERVING box reads 330g. The card was built hardcoding 'g' (see DONE line "Food Detail Stats", AVG SERVING unit split). Use the real serving unit instead of always 'g'. food-detail.tsx. (SOON)
  [ ] BUG: Excluding a day does not refresh the Stats day-by-day calendar immediately -- had to leave the tab and come back to see the change. The excludedDays state needs to reload right after an exclusion toggle. stats.tsx. (SOON)
  [ ] BUG: Home daily-note save button has no haptic. Add medium haptic on save (Toast on Save standard pairs with haptic). index.tsx. (SOON)
