...
NOW -- Calorie Card Denominator Overhaul (Session 62, 2026-05-30)
  ...
  [ ] REQUIRED same session: calories_today tooltip (tooltipRegistry.ts) + cal_card tutorial (data/tutorials.ts) rewrite. Denominator concept changed, RUNNING NET renamed to LIVE NET, and the log bottom row is now 3 stats. Per the tooltip/tutorial sync rule this ships in the same session, not later. Note the existing calories_today tooltip also has a double dash to clean on the Running BMR definition.
