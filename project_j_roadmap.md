...
[x] Tutorial System -- SHIPPED 2026-05-23. Full engine: TutorialContext, useTutorialTarget, TutorialOverlay (4-panel scrim + accent ring + callout bubble + NEXT/SKIP), ToolkitSheet (? icon bottom sheet, per-tab tutorials). 19 tutorials written (meta + 18 feature). All 5 tab headers have ? icon. Meta-tutorial auto-fires first home load. Take a Tour in 7 TooltipModals. Settings > Help > Tutorials lists all 18. Targets register via useTutorialTarget hook -- wired per-card in future sessions. Graceful degradation when targets not mounted.
