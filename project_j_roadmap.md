...
- [ ] Expand Otto's on-demand data access (ONE DATASET AT A TIME, same conditional-injection pattern as PRs / utils/companionPRs.ts): give Otto more of the user's real data, attached only when the message is about it. DONE: PR values + real-exercise recognition + per-lift TREND (A) + recent-workouts B (30-day session summaries, "last chest day" / "how many squat sets this week"). REMAINING candidates, roughly by value: food log history ("what did I eat Tuesday") · detailed sleep history · body measurements · achievements/streak detail · journal/prayer entries (PRIVACY-sensitive, handle carefully). Do NOT build true LLM tool-use yet (Haiku too flaky + extra round-trips); revisit only if Otto grows into a full data assistant on a stronger model. Build/verify each one at a time.
