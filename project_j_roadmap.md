...
## 🏋️ PR / RECORDS WORK (Piece A shipped; these are the open follow-ups)
- [ ] [BUG] Uncheck / edit / delete a set must RECOMPUTE that lift's PR from history and roll it back if the banked best is no longer supported. Right now a PR banks the instant a set is checked and STAYS even if you uncheck it (no revoke). Proper fix (Justin wants it done right, not taped): on set change, recompute the lift's all-time best by scanning setLogs history; if today's unchecked/edited set was what set the best, revert to the true prior best + drop today's prHitsByDay entry + clear the Otto card. Currently we only store the current best (not the chain), so the recompute must derive it from setLogs.
- [ ] PR recap wording cleanup: "New top set: 140 x 5" / "New est. 1RM: 163 lb" reads clunky (workout.tsx recap ~line 2362-2372, both normal + Mindful branches; Otto card body too). Reword to something cleaner like "Heaviest set: 140 lb x 5" / "Est. 1-rep max: 163 lb". KEEP the est. 1-rep max (it IS a standard gym-lifter stat, Justin confirmed keep it) -- just improve the phrasing.
...
- [ ] OPEN Q (Justin never answered): brand-new lift's FIRST-ever session -- if you ramp weight UP across sets (135->155->185), each heavier set beats the prior and shows a "new best" (one card, not spam, but odd to "PR" on a lift's debut). Suppress first-session PRs (only count beating a PRIOR-DAY best), or leave as-is? Existing lifts are unaffected either way.
- [ ] Piece B: the PR HOME. Add a "Your History" section to the exercise detail modal (workout-library.tsx, modal already exists) showing that lift's PR + session history; add a top-right "All PRs" roll-up on the Exercise Library. Data already exists (prs + setLogs + prHitsByDay). When it lands, point the Otto PR card's tap route at it (route was intentionally omitted for now). Decision: NO Stats>Records tile for lifting PRs (Justin declined).
