...
[ ] TUTORIAL POLISH PASS -- dedicated session required. Confirmed good: spotlight animation quality, highlighting, interactive flows, app stability. Issues to fix as a batch:
  ...
  [x] Auto-scroll to off-screen targets -- FIXED 2026-05-26. Root cause: doEntry used single-attempt measureTarget for non-navigation steps. After a preAction, the demo entry ref isn't registered until log.tsx re-renders -- measureTarget returned null, isOffScreen(null)=false, scroll skipped, full-screen dim. Fix: switched non-navigation doEntry branch to measureTargetWithRetry (5 attempts x 150ms), giving React time to re-render and register the ref. Once registered, isOffScreen correctly detects off-screen coords and scrollToTarget fires. TutorialOverlay.tsx.
