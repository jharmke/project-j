...
NOW -- active this session
  ...
  [ ] BUG: Graph data accuracy sweep (net cal) -- sleep score graph fixed 2026-05-29 as part of sleep score inconsistency fix (statsData.ts had same FEEL_BONUS + linear formula bugs). Net calories graph still diverges from day detail for same dates (observed: graph 905 vs day detail 695 -- 210 kcal gap). Suspected root cause: fetchTrendData in utils/statsData.ts uses a stale or zero BMR value at call time, while day detail computes BMR inline from actual profile data loaded fresh on mount. Need to audit utils/statsData.ts net cal computation specifically.
    Remaining sweep: calories consumed, macros (P/C/F), steps, active cals, weight, net cals -- verify each reads correct field from pj_YYYY-MM-DD and applies burn accuracy modifier. Resting HR, respiratory rate, blood oxygen read directly from stored HealthKit fields (lower risk). Net cal is the priority. (NOW)
