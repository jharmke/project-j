...
- [ ] QUICK WINS (small, grab-when-convenient):
  - [ ] FAB TEXT LABELS need the bg-ring too. The FAB circles got the white "bg ring" (borderWidth 3, borderColor bgPrimary) in the global FAB pass, but the expanded sub-action TEXT-LABEL pills ("Load Routine", "Add Exercise", etc.) still blend into same-colored backgrounds. Add the same ring/border to the label pills. Flagged 2026-07-07 (workout tab FAB screenshot). Audit all FABs with text labels. (DONE this session: animate Otto chat bubbles · notification solo-card shadows · Day Summaries month-grouping · EvR saved-reports collapse.)
