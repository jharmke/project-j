...
- [ ] Expand Otto's on-demand data access (ONE DATASET AT A TIME, same conditional-injection pattern as PRs / utils/companionPRs.ts): give Otto more of the user's real data, attached only when the message is about it. DONE: PR values + real-exercise recognition + per-lift TREND (A). REMAINING candidates, roughly by value: recent-workouts B ("what did I do last chest day / how many squat sets this week") · food log history ("what did I eat Tuesday") · detailed sleep history · body measurements · achievements/streak detail · journal/prayer entries (PRIVACY-sensitive, handle carefully). Do NOT build true LLM tool-use yet (Haiku too flaky + extra round-trips); revisit only if Otto grows into a full data assistant on a stronger model. Build/verify each one at a time.
- [ ] [NOW] HIDE the AI-estimator mic (voice dictation was CUT 2026-07-05, no longer building it): the mic (ai-meal-estimator.tsx ~583) still fires a "Voice input is coming soon" toast, so it's now a dead half-feature. Remove/hide the mic button before the next test-group submission so nothing ships half-done.
