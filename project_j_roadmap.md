...
SOON -- confirmed next few sessions
  ...
  [~] Day Score + Day Summary -- SESSION 65 UPDATE (2026-05-31). Engine + pop-up + archive were already shipped across ~17 prior commits (line above was never marked). This session added the FULL SUMMARY PAGE and fixed a real exclusion bug:
        ...
        STILL TO DO (next session): (1) the REAL spotlight tutorial -- rewrite the 6 day_score steps from targetKey:'none' to the baked-in page targets, with a preAction that opens the page (Option C, chosen over wrapping TutorialOverlay in a Modal). (2) Mindful copy/behavior pass for the full page. (3) delete the now-dead archiveSummary DaySummaryModal render + state in stats.tsx (archive routes to the page now). (SOON)
