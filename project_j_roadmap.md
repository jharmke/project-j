...
[ ] Fresh install restore bug -- production build gets fresh app container, 14 local keys from onboarding block restoreIfFresh() auto-trigger. External users will land with no data on first install. Fix: restoreIfFresh() must run even when some local pj_* keys exist (settings/onboarding keys are not real user data), or surface a restore prompt on first login when Firestore has more data than local. MUST FIX before external TestFlight invites go out.
