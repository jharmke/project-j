...
export const TOOLTIP_REGISTRY: TooltipDefinition[] = [
  ...
  },
  {
    ...
    category: 'Nutrition',
    title: 'Repeat a Meal',
    body: 'Eat the same breakfast most mornings? Repeat a Meal re-logs a previous day\'s meal into the day you\'re viewing, so you don\'t re-add the same foods over and over.\n\nWhen a meal slot is empty and you\'ve logged that meal before, the slot reads "Expand to repeat a meal". Expand it and the options are waiting inside. If yesterday\'s same meal has food, one tap on "Repeat Yesterday" copies the whole thing in. To pull from a different day, tap "Pick a Day" (the calendar button next to it) to open the picker; if yesterday is empty the single pill opens the picker itself. There you can choose any day from the last two weeks and check or uncheck individual items before adding.\n\nEverything copies exactly: servings, calories, macros, all the extended nutrition, even the food photo. Nothing is re-searched or re-estimated, so the numbers match the original meal precisely. The foods land as separate entries you can still edit or delete one by one.\n\nLogged the wrong meal? Expand it and tap "Clear all" at the bottom of its item list to remove the whole meal at once.',
    definitions: [
      { term: 'Where it goes', explanation: 'The copied foods always land in the meal slot you opened. You can pull FROM a different meal (say, yesterday\'s Lunch) using the chips at the top of the picker, but they still get added to the slot you started from.' },
