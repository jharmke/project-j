...
export const TOOLTIP_REGISTRY: TooltipDefinition[] = [
  ...
  },
  {
    key: 'repeat_a_meal',
    category: 'Nutrition',
    title: 'Find a Meal, Save as Meal',
    body: 'Eat the same breakfast most mornings? Find a Meal re-logs foods you\'ve eaten before into the day you\'re viewing, so you don\'t re-add the same things over and over. It pulls from two sources: your recent history, and a permanent named list of meals you\'ve saved yourself.\n\nOn an empty meal slot that has recent history or at least one saved meal, the slot reads "Expand to repeat a meal". Expand it and the options are waiting inside. If yesterday\'s same meal has food, one tap on "Repeat Yesterday" copies the whole thing in. "Find a Meal" opens the full picker.\n\nAlready logged something in that slot and want to add more on top? Expand it and "Find a Meal" is right there alongside "Clear all" and "Save as Meal". It\'s never limited to empty slots.\n\nInside the picker there are two tabs. Recent scans the last 14 days and lists every day that has food in the meal you\'re pulling from, newest first. Pick a day, check or uncheck individual items, then add. Meal Catalog is your own saved list: tap "Save as Meal" on any already-logged meal to bundle its current items (uncheck anything you don\'t want first) into a permanent, named meal. Tap a saved meal in the Catalog to expand it the same way, review every item, and add exactly what you check. It never adds everything blind. Delete a saved meal from the Catalog with the trash icon next to its Add button.\n\nEverything copies exactly: servings, calories, macros, all the extended nutrition, even the food photo. Nothing is re-searched or re-estimated, so the numbers match the original meal precisely. The foods land as separate entries you can still edit or delete one by one.\n\nLogged the wrong meal? Expand it and tap "Clear all" at the bottom of its item list to remove the whole meal at once. This never touches a saved meal in your Catalog, only what\'s logged that day.',
    definitions: [
      { term: 'Where it goes', explanation: 'Anything you add always lands in the meal slot you opened Find a Meal from. On the Recent tab you can pull FROM a different meal (say, yesterday\'s Lunch) using the chips at the top, but it still lands in the slot you started from.' },
      { term: 'Recent vs. Meal Catalog', explanation: 'Recent is your rolling history: unnamed, and it ages out after 14 days. Meal Catalog is permanent and named: it only has what you\'ve explicitly saved with Save as Meal, and it never expires.' },
      { term: 'Pick and choose', explanation: 'Whether you\'re saving a meal or adding one back, every item starts checked. Uncheck anything you don\'t want, and the count, calories, and macros update live as you toggle.' },
      { term: 'Clear a whole meal', explanation: 'To wipe everything logged to one meal at once instead of deleting item by item, expand that meal and tap "Clear all" at the bottom. It confirms first, then removes only that meal\'s entries for the day. A saved meal in your Catalog is untouched.' },
    ],
  },
  {
  ...
  },
  {
    ...
    body: 'A PR is a new best, tracked automatically. Most lifts record two things: your heaviest set and your estimated 1-rep max, banked the moment you check a qualifying set. A lift you track as a hold instead of reps (a plank or a loaded carry) banks a longest hold. Cardio synced from Apple Health records two more: your furthest distance and longest duration, tracked separately for each activity (your Outdoor Walking records are their own, apart from Indoor Walking or Cycling). Records stay honest: if the session or set that earned one is changed or removed, it recalculates from your history and rolls back if it is no longer your best.',
    definitions: [
      { term: 'Heaviest set', explanation: 'The most weight you moved for a given number of reps. Beat your previous best on a lift and it sets a record. Shown as weight × reps in that exercise\'s unit, like 140 lb × 5 (or 60 kg × 5 if you log that lift in kilograms).' },
      ...
      { term: 'Longest hold', explanation: 'For a lift you track by time instead of reps (planks, dead hangs, loaded carries, wall sits), your longest hold is the record, shown as minutes:seconds like 1:15. Beat your best hold and it sets a record. On a weighted hold the time is the record and the weight rides along as context, like 0:50 at 32 kg.' },
  ...
  },
  {
    ...
    body: 'These groups track 30 nutrients across 6 categories: carbs, fats, core, vitamins, B vitamins, and minerals. Each value shows your daily total vs your goal.\n\nTap any nutrient to open it. You get which of your logged foods contributed and how much each one added, then Why It Matters and Food Sources: what the nutrient actually does in your body and the everyday foods that carry it. Those two sections are there whether or not you have logged any of it today.\n\nTap the gear icon to set goals. Choose from 5 presets (Standard, Keto, Heart Health, High Fiber, Athletic) or fine-tune any field manually.\n\nColor coding tells you what direction matters for each nutrient. Green means you\'re at or above goal for nutrients you want more of (fiber, vitamins, minerals). Red means you\'ve exceeded a limit for nutrients to keep in check (sodium, saturated fat, trans fat). Accent color means you\'re within range on either type.',
    definitions: [
  ...
  },
  {
    ...
    category: 'Reports',
    title: 'Smart Tip',
    body: 'Smart Tips are pattern-based coaching observations drawn from your logged data over the past 7 to 14 days. They read your patterns across calories, macros (protein, carbs, fat), hydration, sleep, activity, and recovery, then surface the one finding most relevant to you right now.\n\nThe tip here is your highest-priority observation today. Tap the card to open Effort vs Results and see the full picture behind it.\n\nTips update once per day as your data changes. The more consistently you log, the more accurate and specific the tips become.',
    definitions: [
      {
        term: 'Pattern tips',
