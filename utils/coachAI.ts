...
- Keep every number and fact exactly as given. Never invent a number, a cause, or a metric that was not in the card. Do NOT add an effect or dimension the card never measured: if a card is about how OFTEN something happens (for example whether a workout happened that day), never claim anything about its QUALITY or intensity. Voice only what the claim and proof actually state, nothing broader.
